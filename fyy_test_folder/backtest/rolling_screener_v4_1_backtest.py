...
基于 v4-fast 的止损修正版，保留大盘过滤与选股逻辑，
重点修正固定 -8% 止损对高波动强趋势股过紧、对低波股过松的问题。

v4.1 主要变化:
  1. 个股止损线从固定百分比改为 ATR 自适应止损
     - 用买入日前的历史波动估算 ATR%
     - 止损线 = clip(ATR * 倍数, 最紧阈值, 最宽阈值)
     - 默认范围: -5% ~ -12%
  2. 保留原有固定止损模式，便于与 v4 做 A/B 对比
  3. 报告中新增版本说明，明确这是“修正止损方案”

说明:
  - 交易日历和大盘判断都优先使用 index_daily_quotes
  - 默认大盘基准使用沪深300(000300)

使用方法:
    cd ~/Project/TradingAgents-CN && source venv/bin/activate

    # 默认参数: v4.1 ATR 自适应止损
    python fyy_test_folder/backtest/rolling_screener_v4_1_backtest.py \
...
def get_recent_price_rows(db, symbol: str, end_date: str, limit: int = 40) -> List[Dict]:
    """获取截止到指定日期的最近若干根 K 线，用于 ATR 止损估算。"""
    cursor = db.stock_daily_quotes.find(
...
def run_rolling_backtest(
    ...
    if stop_mode == "atr":
        logger.info(
            "止损模式: ATR 自适应 | 基线 %.1f%% | ATR窗口 %d | ATR倍数 %.2f | 范围 %.1f%% ~ %.1f%% | 大盘基准: %s",
            stop_loss_pct * 100,
            atr_window,
            atr_multiplier,
        ...
        logger.info("止损模式: 固定止损 %.1f%% | 大盘基准: %s", stop_loss_pct * 100, regime_benchmark)
    ...
    for rd, sel_date in enumerate(selection_dates, 1):
        ...
        for code, name, comp_score, tag in codes:
            ...
            atr_pct = None
            if stop_mode == "atr":
                # 只允许使用选股日及之前的数据估算 ATR，避免把买入日完整 K 线带入止损线。
                stop_rows = get_recent_price_rows(db, code, sel_date, limit=max(atr_window + 5, 40))
...
def print_and_export(
    ...
    print("=" * 80)
    print("  滚动选股回测 v4.1 结果  ".center(80, "="))
    print("=" * 80)
    print("\n【每轮汇总】")
    ...
    det_path = out_dir / f"rolling_v4_1_detail_{ts}.csv"
    sum_path = out_dir / f"rolling_v4_1_summary_{ts}.csv"
    rpt_path = out_dir / f"rolling_v4_1_report_{ts}.json"
    ...
    report = {
        "version": "v4.1",
        "description": "修正止损方案：将固定百分比止损改为 ATR 自适应止损，降低高波动强趋势股被过早止损的概率。",
        "timestamp": ts,
        "params": {
...
def main():
    parser = argparse.ArgumentParser(description="滚动选股回测 v4.1（大盘过滤 + 修正止损方案）")
    ...
    parser.add_argument("--stop-mode", choices=["atr", "fixed"], default=DEFAULT_STOP_MODE,
    ...
    parser.add_argument("--min-stop-loss", type=float, default=-12,
                        help="ATR 自适应止损的最宽阈值百分比 (默认 -12)")
    parser.add_argument("--max-stop-loss", type=float, default=-5,
                        help="ATR 自适应止损的最紧阈值百分比 (默认 -5)")
